...
* Perform the graphical installation remotely by connecting to the installation system from another computer with a graphical display using the _VNC_ (Virtual Network Computing) protocol as described in xref:advanced-install:assembly_performing-a-remote-installation-using-vnc.adoc[Performing a remote RHEL installation using VNC]
